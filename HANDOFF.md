...
**All content is plain files in this repo.** Lessons and documentation are
Markdown, team text is JSON, photos are image files. There is no database and no server.
...
- **CMS stops working**: content can always be edited directly on GitHub
  (open the file in `src/content/docs/lessons/` or
  `src/content/docs/documentation/`, click the pencil). The CMS is a
...
- **Two theme toggles by design**: the marketing pages' Light/Dark button
  and the Starlight theme picker (shared by lessons and documentation)
  remember their settings independently. Not a bug.
- **Lessons and documentation must use plain Markdown, not Starlight
  asides.** The CMS (Keystatic) can't parse Starlight's `:::note` / `:::tip` /
  `:::caution` block syntax — a page that contains one won't appear in the CMS. Use a
  ...
  files.
- **Two image pipelines by design**: lesson and documentation body images
  (inserted via the CMS MDX editor) are stored in
  `public/lesson-images/<subteam>/<slug>/` and
  `public/documentation-images/<subteam>/<slug>/` respectively, and served
...
- **Two Starlight sections, one instance.** `/lessons/` and `/documentation/`
  are the same Starlight site with two autogenerated sidebar groups (see
  `sidebar` in `astro.config.mjs`), one shared theme file
  (`src/styles/starlight.css`), and one shared search index. They're structurally
  identical on purpose — anything this document says about lessons applies to
  documentation too.
- **Sidebar groups come from slug prefixes.** The collections use
  `path: 'src/content/docs/<section>/<subteam>/**'` (note the `**`), so a slug with
